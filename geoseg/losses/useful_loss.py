...
class UnetFormerLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            ...
            loss = self.main_loss(logit_main, labels) + 0.4 * self.aux_loss(logit_aux, labels)  #0.4
        else:
            loss = self.main_loss(logits, labels)

        return loss
        
class LOG_SACLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            logit_main, logit_aux = logits
            loss = self.main_loss(logit_main, labels) + 0.8 * self.aux_loss(logit_aux, labels)
        else:
            loss = self.main_loss(logits, labels)
