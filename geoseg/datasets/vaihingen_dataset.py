...
def get_training_transform():
    train_transform = [
        albu.RandomRotate90(p=0.5),  # 这个变换会随机地将图像旋转90度、180度、270度或者不进行任何旋转。`p=0.5` 表示应用此变换的概率为50%。
        albu.Normalize()  # 这个变换会对图像进行标准化，通常是将图像的像素值缩放到一个特定的范围（例如：[0,1] 或 [-1,1]），并可能调整其均值和标准差。
...
def train_aug(img, mask):
    # RandomScale 用于随机缩放图像和掩码，缩放的比例可以是列表中的任意一个值（0.5, 0.75, 1.0, 1.25, 1.5）
    # SmartCropV1 用于智能裁剪图像和掩码到指定大小（512x512）
    # max_ratio 参数限制了裁剪时长宽比的最大变化，ignore_index 是用于标记忽略区域的类别索引
...
class VaihingenDataset(Dataset):
    def __init__(self, data_root='data/vaihingen/test', mode='val', img_dir='images_1024', mask_dir='masks_1024',
        ...
        self.img_suffix = img_suffix  # 图像文件的文件后缀
        self.mask_suffix = mask_suffix
        self.transform = transform  # 应用于图像和掩码的变换函数
        self.mode = mode
        self.mosaic_ratio = mosaic_ratio
        self.img_size = img_size
        self.img_ids = self.get_img_ids(self.data_root, self.img_dir, self.mask_dir)  # 获取图像ID
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        # split('.')方法来分割文件名和文件扩展名
        img_ids = [str(id.split('.')[0]) for id in mask_filename_list]  # 提取图像ID
        return img_ids

    # 根据提供的索引从数据集中加载对应的图像和掩码
    def load_img_and_mask(self, index):
        img_id = self.img_ids[index]
        img_name = osp.join(self.data_root, self.img_dir, img_id + self.img_suffix)
        mask_name = osp.join(self.data_root, self.mask_dir, img_id + self.mask_suffix)
        img = Image.open(img_name).convert('RGB')  # convert('RGB')确保图像被转换为RGB模式
        mask = Image.open(mask_name).convert('L')  # convert('L')将掩码转换为灰度模式（L代表灰度）
        return img, mask

    # 将四幅图像及其掩码组合成一个马赛克图像
    def load_mosaic_img_and_mask(self, index):
        # 指定的图像外，其他三幅图像都是随机选取的
        ...
        offset_x = random.randint(start_x, (w - start_x))  # 偏移量
